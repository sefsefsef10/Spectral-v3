...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/webhooks/langsmith/:aiSystemId", async (req, res) => {
    try {
      ...
      const WEBHOOK_SECRET = process.env.LANGSMITH_WEBHOOK_SECRET || "spectral-dev-secret-123";
              ...
              if (aiSystem) {
                ...
                if (healthSystem) {
                  await sendCriticalAlertEmail(
                    'admin@example.com', // TODO: Fetch admin users from users table
                    'Administrator',
                    {
